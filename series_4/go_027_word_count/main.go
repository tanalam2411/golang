...
var WordsSlice = []string{
	...
	`For example, given 12.345 the format %6.3f prints 12.345 while %.3g prints`,
	`12.3. The default precision for %e and %f is 6; for %g it is the smallest`,
	...
	``,
	`Scanf, Fscanf and Sscanf require that (after skipping spaces) newlines in`,
	`the format are matched by newlines in the input and vice versa. This`,
	`behavior differs from the corresponding routines in C, which uniformly treat`,
	`newlines as spaces.`,
	``,
	`When scanning with Scanf, Fscanf, and Sscanf, all non-empty runs of space`,
	`characters (except newline) are equivalent to a single space in both the`,
	`format and the input. With that proviso, text in the format string must`,
	`match the input text; scanning stops if it does not, with the return value`,
	...
	`hexadecimal number, and %v will scan the default representation format for`,
	`the value.`,
	``,
	`The formats behave analogously to those of Printf with the following`,
	`exceptions:`,
	``,
...
func main() {
	...
	lineCh := reader(WordsSlice)

	wordsCh := lineSplitter(lineCh)

	wg.Add(1)
	go wordCounter(&result, wordsCh, &wg)
	wg.Wait()


	for count, word := range result{
...
func wordCounter(result *map[string]int, wordsCh <-chan []string, wg *sync.WaitGroup) {
	// if word is not in result, result[word] will return 0 // will not throw an error
