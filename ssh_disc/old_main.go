...
func sshExecutor(ip, username, password string, commands []string, credentials map[string]string) error {
	...
	fmt.Printf("Trying ip: %v \n", ip)
	client, err := getSSHClient(ip, credentials)
	fmt.Printf("ip: %v, client: %v, err: %v \n", ip, client, err)
...
func getSSHClient(ip string, credentials map[string]string) (ssh.Client, error) {
	...
	//timeout := time.After(time.Duration(sshTimeOut * 2) * time.Second)

	for username, password := range credentials {

		go authenticate(ip, username, password, sshTimeOut, clientChan, errorChan)

	}

	client := ssh.Client{}
	err := errors.New("Failed to connect using ssh")
...
func authenticate(host, username, password string, timeout int, clientChan chan *ssh.Client, errorChan chan error) {
	...
	//fmt.Printf("Dailing: %v", host)
	client, err := ssh.Dial("tcp", host+":22", config)

	if err != nil {
		fmt.Printf("Failed to connect ip:%v, username:%v, password:%v \n\n", host, username, password)
		clientChan <- nil
		errorChan <- err
		//return ssh.Client{}, err
	} else {
		fmt.Printf("Success ip:%v, username:%v, password:%v \n\n", host, username, password)
		clientChan <- client
		errorChan <- err
	}
